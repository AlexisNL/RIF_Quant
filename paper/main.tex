...
\maketitle
\maketitle
\blfootnote{Version: February 2026. Preliminary and Incomplete. Working Paper --- Comments Welcome. E-mail: \texttt{anoirluhalwe@yahoo.com}}
% ========== ABSTRACT ==========
\begin{abstract}
...
The resulting local regime distributions confirm that regime structure is fundamentally asset-specific. GOOG spends 95.1\% of the trading day in a single dominant regime with only brief excursions into stress states (1.3\% and 3.6\%), consistent with its relatively low intraday volatility on this date. INTC exhibits a similarly concentrated structure (80.6\% in the dominant regime) but with a more substantial secondary regime (13.3\%). At the other extreme, MSFT distributes its time across regimes more evenly (7.5\%/29.3\%/63.2\%), suggesting richer microstructure dynamics with more frequent regime transitions. AAPL falls between these extremes (56.9\%/36.6\%/6.5\%), while AMZN mirrors INTC's concentrated structure with a different secondary allocation (11.4\%).

Figures~\ref{fig:posterior_aapl}--\ref{fig:posterior_msft} present the posterior regime probabilities for each ticker as stacked area plots, providing a continuous view of regime assignment confidence throughout the trading day. The color coding reflects the economic interpretation of each regime per ticker (green $=$ calm, blue $=$ intermediate, red $=$ stressed), determined by the mean Wasserstein feature profiles documented below.

\begin{itemize}
\item \textbf{AAPL} (Figure~\ref{fig:posterior_aapl}). Regime 0 (56.9\%, green/calm) is a low-stress baseline; Regime 1 (36.6\%, blue/intermediate) is an intermediate state with broader increases across Price/OFI/OBI; Regime 2 (6.5\%, red/stressed) concentrates the strongest joint Price--OFI--OBI ruptures, consistent with synchronized microstructure stress.
\item \textbf{INTC} (Figure~\ref{fig:posterior_intc}). Regime 0 (80.6\%, green/calm) is dominant and comparatively calm; Regime 1 (13.3\%, blue/intermediate) is imbalance-dominant (OBI peak); Regime 2 (6.1\%, red/stressed) is flow-price dominant (OFI and Price peaks), indicating two distinct stress channels.
\item \textbf{GOOG} (Figure~\ref{fig:posterior_goog}). Regime 0 (95.1\%, green/calm) reflects persistent baseline conditions; the rare Regime 1 (1.3\%, red/stressed) is OFI-shock dominated; Regime 2 (3.6\%, blue/intermediate) is OBI-surge dominated, separating flow toxicity from book-skew episodes.
\item \textbf{AMZN} (Figure~\ref{fig:posterior_amzn}). Regime 0 (83.9\%, green/calm) is the low-stress background; Regime 1 (4.7\%, red/stressed) concentrates the strongest joint OFI--OBI stress; Regime 2 (11.4\%, blue/intermediate) captures strong Price--OFI ruptures with more moderate imbalance.
\item \textbf{MSFT} (Figure~\ref{fig:posterior_msft}). Regime 0 (7.5\%, red/stressed) concentrates the strongest Price--OBI disturbances; Regime 1 (29.3\%, blue/intermediate) is OFI-led activity with limited price dislocation; Regime 2 (63.2\%, green/calm) behaves as the baseline low Price/OFI state.
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/hmm_local_AAPL_posterior.png}
\caption{Posterior regime probabilities for AAPL. Green $=$ calm (R0), blue $=$ intermediate (R1), red $=$ stressed (R2).}
...
\caption{Posterior regime probabilities for INTC. Green $=$ calm (R0), blue $=$ intermediate (R1), red $=$ stressed (R2).}
...
\caption{Posterior regime probabilities for GOOG. Green $=$ calm (R0), red $=$ stressed (R1), blue $=$ intermediate (R2).}
...
\caption{Posterior regime probabilities for AMZN. Green $=$ calm (R0), red $=$ stressed (R1), blue $=$ intermediate (R2).}
...
\caption{Meta-HMM global posterior regime probabilities throughout the trading day.}
...
\caption{Direct Global HMM posterior regime probabilities throughout the trading day.}
