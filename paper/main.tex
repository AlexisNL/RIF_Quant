...
I apply the framework to LOBSTER data for five technology stocks (AAPL, INTC, GOOG, AMZN, MSFT) on June 21, 2012 to answer a simple question: does one sector behave as one market, or as several micro-markets that occasionally align? The evidence points to a mixed structure. At the single-stock level, each name follows its own regime geometry with different optimal MAD/Wasserstein windows and persistence; some tickers concentrate the vast majority of the trading day in a single dominant regime, while others distribute time more evenly across states. Yet at the sector level, two conceptually different global constructions---Meta-HMM and Direct Global HMM---detect broadly similar structures despite producing complementary rather than redundant regime characterizations: the Meta-HMM better captures the cluster geometry of the sector-wide feature space, while the Direct model achieves stronger distributional separation between its identified regimes. Local-global synchronization remains low on this non-crisis day, consistent with limited coupling and providing a natural baseline against which crisis-day dynamics can be compared. Directionality analysis reveals that co-movement is organized rather than symmetric: AAPL acts as a pure anticipatory initiator whose local transitions consistently precede global shifts without directly co-transitioning, GOOG as the dominant direct co-transition partner, INTC as a secondary anticipatory amplifier whose causal role intensifies markedly under stress, and AMZN as a net follower. Transfer Entropy maps the directed information network: AAPL emerges as the dominant information hub receiving the largest inflows from its peers, with its strongest outgoing channel directed toward MSFT. Lead-lag analysis documents a state-dependent topology in which sector leadership inverts between calm and stressed regimes: the most liquid name sets the directional tone during quiescent periods, while INTC becomes the dominant anticipatory leader under high stress---a pattern invisible to static correlation measures. An intraday GOOG order-flow event illustrates the reallocation mechanism in real time: a localized shock in one name triggers synchronous order-flow responses across the sector, consistent with active portfolio rebalancing rather than passive contagion. Overall, the main takeaway is that sector dynamics are hierarchical---idiosyncratic locally, partially coherent globally, and connected through time-varying directed links.
...
The dominant regime is R2 (Calm, 52.0\%), with R1 (Stressed, 34.6\%) as a substantial secondary state and R0 (Intermediate, 13.4\%) as the minor regime. Return volatility is the primary discriminant: the Stressed regime R1 carries substantially higher $\sigma_r$ (0.73\,bp) and $|\bar{r}|$ (0.37\,bp) than Calm R2 (0.49 and 0.24\,bp), while $|\overline{\text{OBI}}|$ and $|\overline{\text{OFI}}|$ differ more modestly across regimes. The high kurtosis of Stressed R1 (62.2) versus Calm R2 (15.5) indicates that the stressed regime is driven by sparse, large return spikes rather than sustained elevated volatility.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/hmm_local_AAPL_posterior.png}
\caption{Viterbi regime sequence for AAPL (stacked area, one-hot encoded). Green $=$ Calm (R2, 52.0\%), blue $=$ Intermediate (R0, 13.4\%), red $=$ Stressed (R1, 34.6\%). Color assignment based on the composite stress proxy of Equation~\eqref{eq:stress_proxy}.}
\label{fig:posterior_aapl}
\end{figure}

\item \textbf{INTC} (Table~\ref{tab:regime_stats_intc}).
INTC exhibits a highly concentrated structure: R0 (Calm, 81.1\%) dominates, with R2 (Intermediate, 12.8\%) and R1 (Stressed, 6.1\%) as smaller regimes. Notably, return volatility is non-monotonic ($\sigma_r = 0.60, 0.64, 0.40$\,bp for Calm, Intermediate, and Stressed), so OBI drives the stress ordering: R1 (Stressed) carries the highest $|\overline{\text{OBI}}|$ (0.12) and remains elevated on $|\overline{\text{OFI}}|$ (385). The very high return kurtosis across all regimes (94--132) indicates that INTC's return distribution is uniformly fat-tailed regardless of regime---a signature of its thinner market structure relative to AAPL.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/hmm_local_INTC_posterior.png}
\caption{Viterbi regime sequence for INTC. Green $=$ Calm (R0, 81.1\%), blue $=$ Intermediate (R2, 12.8\%), red $=$ Stressed (R1, 6.1\%).}
...
AMZN's dominant regime is R0 (Intermediate, 66.3\%), with R1 (Calm, 22.4\%) and R2 (Stressed, 11.3\%) as smaller regimes. Return volatility is non-monotonic: R0 (Intermediate) carries the highest $\sigma_r$ (0.90\,bp) and $|\bar{r}|$ (0.40\,bp), while R2 (Stressed) achieves its label through the highest $|\overline{\text{OFI}}|$ (242 vs.\ 182 for Intermediate and 174 for Calm). This pattern suggests that AMZN's stress ordering is driven by order-flow pressure rather than price volatility: the Stressed regime is characterized by directional order-book activity consistent with institutional order-slicing rather than by large price moves.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/hmm_local_AMZN_posterior.png}
\caption{Viterbi regime sequence for AMZN. Green $=$ Calm (R1, 22.4\%), blue $=$ Intermediate (R0, 66.3\%), red $=$ Stressed (R2, 11.3\%). Stress ordering is driven by $|\overline{\text{OFI}}|$ (242 for R2) rather than return volatility.}
\label{fig:posterior_amzn}
\end{figure}

\item \textbf{MSFT} (Table~\ref{tab:regime_stats_msft}).
MSFT's dominant regime is R2 (Intermediate, 96.6\%), flanked by a near-quiescent R0 (Calm, 1.4\%) and a rare Stressed episode R1 (2.0\%). Return volatility provides a clear and monotonic gradient ($\sigma_r = 0.04, 0.63, 0.74$\,bp for Calm, Intermediate, and Stressed), making it the primary discriminant. Both $|\overline{\text{OBI}}|$ and $|\overline{\text{OFI}}|$ also increase monotonically across regimes (0.05, 0.08, 0.11 and 379, 412, 420 respectively), confirming coherent stress ordering. The Calm regime R0 is exceptionally quiescent ($\sigma_r = 0.04$\,bp), consistent with a near-zero-volatility intraday episode.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/hmm_local_MSFT_posterior.png}
\caption{Viterbi regime sequence for MSFT. Green $=$ Calm (R0, 1.4\%), blue $=$ Intermediate (R2, 96.6\%), red $=$ Stressed (R1, 2.0\%). Regime separation is driven by return volatility ($\sigma_r = 0.04, 0.63, 0.74$\,bp for Calm, Intermediate, Stressed).}
...
The Meta-HMM and Direct Global HMM identify sector-wide regimes from fundamentally different inputs---local state probabilities versus raw Wasserstein features---and produce regime sequences with limited raw state overlap (8.1\%, Table~\ref{tab:robustness_unified}). This low agreement should not be interpreted as inconsistency: the ARI between the two models is 0.281, indicating fair structural agreement once label permutations are accounted for, while the raw 8\% reflects the different regime boundary placements each model adopts. Both models produce near-deterministic assignments (mean entropy 0.0007 and 0.0009 respectively), confirming that regime uncertainty is not the source of disagreement. Rather, the two approaches segment the same underlying dynamics differently: the Meta-HMM, operating on filtered local probability simplices, is strongly aligned with K-means structure (ARI $= 0.476$), while the Direct model, operating on raw Wasserstein features, reaches higher maximum distributional separation between regimes (MMD$_{\text{OFI}} = 1.021$ vs.\ $0.523$) at the cost of lower K-means alignment (ARI $= 0.150$). The Meta-HMM thus captures the broader cluster geometry of the feature space, while the Direct model prioritizes maximal distributional distance between its identified regimes.

Synchronization between each ticker's local HMM and the Meta-HMM global regime is reported in Appendix~\ref{app:local_global_sync} (Table~\ref{tab:local_global_sync}), across 39 global transitions. Two distinct patterns emerge. Anticipatory tickers (AAPL and INTC) exhibit zero raw co-transition synchronization---their local transitions never coincide with the global transition window---yet carry high leadership scores (1.0 and 0.5 respectively), meaning their local state changes reliably \textit{precede} global shifts. GOOG is the ticker most directly coupled to global dynamics: it combines the highest co-transition synchronization rate (17.9\%, 7 co-transitions out of 39) with a leadership score of 1.0, indicating that all its synchronized transitions precede rather than lag the global shift. MSFT shows moderate coupling (10.3\%, 4 co-transitions) with a net leadership score of 0.333 (14 leads, 7 lags). AMZN is a net follower: leadership score 0.0, with near-zero synchronization (2.6\%, 1 co-transition).
...
Table~\ref{tab:transfer_entropy} (reported in Appendix~\ref{app:contagion}) presents the top directed information flow relationships between tickers, computed from regime posterior probabilities. The dominant individual link is GOOG$\rightarrow$AAPL (TE $= 5.4\times10^{-4}$ nats), followed by MSFT$\rightarrow$AAPL ($4.0\times10^{-4}$), AMZN$\rightarrow$AAPL ($3.3\times10^{-4}$), AAPL$\rightarrow$MSFT ($3.3\times10^{-4}$), and INTC$\rightarrow$AAPL ($3.3\times10^{-4}$). AAPL emerges as the dominant information hub: four of the five largest flows target AAPL, indicating that AAPL's regime state is heavily informed by the rest of the sector. Among AAPL's outgoing flows, AAPL$\rightarrow$MSFT ($3.3\times10^{-4}$ nats) is the strongest, and MSFT is bidirectional---receiving from AAPL while also sending back ($4.0\times10^{-4}$ nats). Average outgoing TE values are homogeneous across tickers ($1.80$--$2.01\times10^{-4}$ nats mean), so AAPL's first-place contagion rank is driven primarily by its leadership score (1.0) rather than a dominant outgoing TE advantage.

Combining the TE matrix with synchronization leadership scores identifies AAPL as the leading Patient Zero candidate (composite contagion score 2.0, highest among all tickers). Its leadership score of 1.0 indicates that all local regime transitions precede global shifts, yet its co-transition synchronization is 0\% (0 out of 39), marking AAPL as a pure \emph{anticipatory initiator} rather than a simultaneous co-transition trigger. GOOG presents a complementary profile: second-highest contagion score (1.47), leadership score 1.0, and the highest co-transition synchronization (17.9\%, 7 out of 39), consistent with both anticipation and direct co-transition behavior. The dominant outgoing channel from AAPL is AAPL$\rightarrow$MSFT ($3.3\times10^{-4}$ nats), while the single largest flow in the network is GOOG$\rightarrow$AAPL ($5.4\times10^{-4}$ nats). AMZN, by contrast, has a leadership score of 0.0, consistent with a net follower role. INTC contributes as a secondary amplifier, particularly in the high-stress Q90 lead-lag network where it leads AAPL by $7.5$\,s with $r = 0.526$. This layered structure---AAPL as anticipatory initiator, GOOG as co-transition trigger, MSFT as primary TE recipient, INTC as stress amplifier---provides a richer picture of intra-sector contagion than symmetric correlation analysis.
...
Table~\ref{tab:robustness_unified} (reported in Appendix~\ref{app:robustness}) consolidates all robustness diagnostics. The Meta-HMM is substantially better aligned with the non-parametric K-means benchmark (ARI $= 0.476$) than the Direct Global HMM (ARI $= 0.150$), providing strong evidence that the hierarchical architecture captures the genuine cluster geometry of the sector-wide feature space. The meta-vs-direct ARI of 0.281 indicates fair structural agreement between the two global models despite their differing regime boundaries. Moderate ARI values are structurally expected: HMM persistence constraints partition the feature space differently from K-means, so the ARI measures the overlap between a time-persistent labelling and a memoryless one. A high ARI therefore confirms genuine distributional cluster structure; the Meta-HMM's 0.476 is notably strong in this respect.

The Maximum Mean Discrepancy diagnostics reveal a complementary picture. The Direct Global HMM achieves substantially stronger distributional separation between regimes---MMD$_{\text{OFI}} = 1.021$ vs.\ $0.523$ for the Meta-HMM, and MMD$_{\text{OBI}} = 0.573$ vs.\ $0.455$---indicating that the Direct model's regime boundaries are positioned to maximize feature-space divergence. The Meta-HMM sacrifices some distributional sharpness for better K-means alignment, reflecting the information compression induced by the hierarchical architecture. Price separation is moderate in both models (0.172 and 0.235), consistent with the interpretation that price dynamics are less regime-discriminant than order-flow and imbalance signals. Both models assign near-deterministic regimes (mean entropy 0.0007 and 0.0009), confirming that the low state-agreement between the two models reflects genuine regime boundary disagreement rather than probabilistic ambiguity. Detailed local-level diagnostics and ARI comparisons are in Appendix~\ref{app:robustness}.
...
The weak but structured global synchronization is consistent with a non-crisis day. The low synchronization rates (0--17.9\%) and moderate ARI (0.150--0.476) indicate that idiosyncratic microstructure dynamics dominate sector-wide co-movement. The discovery that AAPL is a pure anticipatory initiator (leadership 1.0, zero co-transitions) while GOOG is the dominant direct co-transition partner (17.9\% sync, leadership 1.0) and AMZN is a net follower (leadership 0.0) provides a nuanced baseline: on a crisis day, one would expect this layered structure to compress into broader simultaneous co-transitions as common shocks override idiosyncratic dynamics.

Transfer Entropy reveals that AAPL is the dominant information hub, receiving the four largest flows in the network (GOOG$\rightarrow$AAPL, MSFT$\rightarrow$AAPL, AMZN$\rightarrow$AAPL, INTC$\rightarrow$AAPL). Its strongest outgoing link is toward MSFT (AAPL$\rightarrow$MSFT). MSFT is bidirectional with AAPL, consistent with its position as the most broadly held institutional stock in the dataset. The directed TE structure, combined with the stress-conditioned lead-lag evidence that INTC leads AAPL by $7.5$\,s at Q90, reveals a causal topology that is both asymmetric and state-dependent---neither of which is detectable from unconditional correlation matrices.
...
Empirically, the framework identifies heterogeneous local regime structures across five technology stocks, with per-ticker parameter optimization revealing that no single configuration adequately captures the diverse microstructure signatures within a sector. Global synchronization is weak but structured, consistent with moderate intra-sector dependence on a non-crisis day and providing a natural baseline for crisis-day comparison. Transfer Entropy identifies a dominant information hub that receives the largest directed flows from its peers while directing its strongest outgoing channel toward the most institutionally held name in the sector; Patient Zero identification reveals a pure anticipatory initiator---whose local transitions consistently precede global shifts without direct co-transitioning---alongside a direct co-transition partner, with the remaining tickers ranging from secondary amplifiers to net followers. Lead-lag analysis documents a state-dependent topology in which sector leadership inverts between calm and stressed regimes, with the causal influence of a thinner-market name intensifying markedly under high stress---a phenomenon invisible to static correlation measures. An event study of an intraday order-flow event confirms hierarchical capital reallocation: a localized shock triggers synchronized responses across the sector, consistent with active portfolio rebalancing rather than passive spillover.
