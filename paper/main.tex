...
I apply the framework to LOBSTER data for five technology stocks (AAPL, INTC, GOOG, AMZN, MSFT) on June 21, 2012 to answer a simple question: does one sector behave as one market, or as several micro-markets that occasionally align? The evidence points to a mixed structure. At the single-stock level, each name follows its own regime geometry, with different optimal MAD/Wasserstein windows and persistence; GOOG remains almost entirely in one dominant state (95.1\%), while MSFT concentrates in a single dominant state (78.1\%) flanked by two smaller regimes (3.3\% and 18.6\%). Yet at the sector level, two conceptually different global constructions (Meta-HMM and Direct Global HMM) detect broadly similar structures (ARI $= 0.281$), though their raw state boundaries differ substantially (8.1\% overlap), indicating complementary rather than redundant representations. This common signal is not constantly shared: local-global synchronization stays low (0--10.8\%), consistent with limited coupling on a non-crisis day. Directionality analysis then shows that co-movement is organized rather than symmetric and exhibits a two-tier structure: AMZN and INTC are pure early-warning indicators (leadership scores 0.60 and 0.50) that fire before sector-wide transitions without co-transitioning, while AAPL directly triggers global shifts as evidenced by both the highest co-transition synchronization (10.8\%) and the dominant outgoing TE channel AAPL$\rightarrow$MSFT (0.0060 nats). Lead-lag analysis reveals a state-dependent network topology: GOOG leads AAPL by 2\,s in calm conditions ($r = 0.393$ at Q10), while INTC becomes the dominant sector leader under high stress, anticipating AAPL by 7.5\,s ($r = 0.526$ at Q90). The intraday GOOG OFI spike around $t \approx 15{,}000$ ($\sim$11:33 AM), followed by coordinated cross-sectional reallocation, illustrates this mechanism in real time: shocks are absorbed through active rebalancing channels, not passive spillover. Overall, the main takeaway is that sector dynamics are hierarchical---idiosyncratic locally, partially coherent globally, and connected through time-varying directed links.
...
\begin{equation}
\widetilde{\sigma}_t = \text{norm}_{[0,1]}\!\left(|\Delta p_t / p_t|\right) + \text{norm}_{[0,1]}\!\left(|\text{OBI}_t|\right) + \text{norm}_{[0,1]}\!\left(|\text{OFI}_t|\right)
\label{eq:stress_proxy}
\end{equation}
where each metric is independently scaled to $[0,1]$ over the full series so that no single dimension dominates due to unit differences. The per-regime mean $\bar{\widetilde{\sigma}}_r = \mathbb{E}[\widetilde{\sigma}_t \mid S_t = r]$ determines the rank ordering: the regime with the lowest $\bar{\widetilde{\sigma}}_r$ is labelled Calm (green), the middle Intermediate (blue), and the highest Stressed (red). Detailed descriptive statistics are provided in Appendix~\ref{app:regime_stats}.
...
\item \textbf{AAPL} (Figure~\ref{fig:posterior_aapl}, Table~\ref{tab:regime_stats_aapl}).
The dominant regime is R1 (Intermediate, 79.4\%), flanked by a rare calm baseline R0 (2.3\%) and a secondary stress episode R2 (18.3\%). Regime separation is primarily driven by order-book and flow dimensions: the Stressed regime R2 carries the highest $|\overline{\text{OBI}}|$ (0.37 vs.\ 0.26 for Calm) and $|\overline{\text{OFI}}|$ (276 vs.\ 218), while return volatility $\sigma_r$ differs more modestly (0.48--0.60\,bp). The kurtosis of Intermediate R1 (61.6) substantially exceeds that of R0 and R2, indicating that the large majority of the trading day is punctuated by occasional extreme return events despite moderate average volatility.

\item \textbf{INTC} (Figure~\ref{fig:posterior_intc}, Table~\ref{tab:regime_stats_intc}).
INTC exhibits a highly concentrated structure: R0 (Calm, 80.6\%) dominates, with R1 (Intermediate, 13.3\%) and R2 (Stressed, 6.1\%) accounting for a small fraction of the day. The Stressed regime is unambiguous: R2 leads on all three dimensions simultaneously ($\sigma_r = 0.77$\,bp, $|\overline{\text{OBI}}| = 0.13$, $|\overline{\text{OFI}}| = 402$). Notably, both R1 and R2 display very high return kurtosis ($\approx 110$), indicating that even the intermediate and stressed regimes feature sparse, large return spikes rather than sustained elevated volatility---a signature of INTC's thin market structure relative to AAPL.

\item \textbf{GOOG} (Figure~\ref{fig:posterior_goog}, Table~\ref{tab:regime_stats_goog}).
GOOG's regime structure is atypical: a single dominant state R0 covers 95.1\% of the trading day, with brief excursions into R1 (1.3\%) and R2 (3.6\%). By the composite stress proxy, R0 has the highest intraday microstructure activity ($\sigma_r = 0.68$\,bp, $|\overline{\text{OBI}}| = 0.31$), while R1 is the calmest ($\sigma_r = 0.37$\,bp, $|\overline{\text{OFI}}| = 145$) and R2 stands out through its elevated OFI ($|\overline{\text{OFI}}| = 219$, mean signed OFI $= -45.9$ shares: persistent sell-flow pressure). The dominant R0 being the most active state does not indicate ongoing market distress; rather, it reflects GOOG's characteristically high intraday microstructure intensity---a highly liquid mega-cap where typical conditions involve substantial price discovery and book imbalance. R1 and R2 represent brief episodes of comparatively quiescent or directionally extreme conditions. This concentrated structure suggests that the Wasserstein features for GOOG contain less cross-regime variation than for other tickers, a limitation acknowledged by the relatively modest ARI score ($0.735$) despite the optimal parameters.

\item \textbf{AMZN} (Figure~\ref{fig:posterior_amzn}, Table~\ref{tab:regime_stats_amzn}).
AMZN's Calm baseline is R2 (84.1\%), with Intermediate R0 (9.9\%) and Stressed R1 (6.0\%). The Stressed regime R1 is the most clearly demarcated across all five tickers: its return volatility ($\sigma_r = 1.09$\,bp) is nearly double that of R2 (0.86\,bp) and 80\% above R0 (0.61\,bp), constituting a volatility shock rather than a gradual elevation. Interestingly, R0 (Intermediate) carries the highest $|\overline{\text{OBI}}|$ (0.39) and $|\overline{\text{OFI}}|$ (232), meaning the Intermediate regime is characterized more by directional order-book pressure than by price volatility---a pattern consistent with informed institutional trading building positions without yet triggering large price moves.

\item \textbf{MSFT} (Figure~\ref{fig:posterior_msft}, Table~\ref{tab:regime_stats_msft}).
MSFT's dominant regime is R1 (Intermediate, 78.1\%), followed by R0 (Stressed, 18.6\%) and R2 (Calm, 3.3\%). Return volatility is nearly identical across all three regimes ($\sigma_r \in [0.56, 0.63]$\,bp), making it the weakest discriminant. The principal source of regime separation is $|\overline{\text{OFI}}|$: the Stressed regime R0 carries a substantially higher flow imbalance ($|\overline{\text{OFI}}| = 484$) than Intermediate R1 (395) and Calm R2 (403). The high kurtosis across all regimes (52--64) indicates that MSFT's return distribution is uniformly fat-tailed regardless of regime, suggesting that occasional large price moves are a structural feature of MSFT's microstructure rather than a regime-specific phenomenon. The weak stress separation observed here is reflected in the optimization score: MSFT achieves the lowest ARI ($0.581$) among the five tickers.
...
\caption{Viterbi regime sequence for AAPL (stacked area, one-hot encoded). Green $=$ Calm (R0, 2.3\%), blue $=$ Intermediate (R1, 79.4\%), red $=$ Stressed (R2, 18.3\%). Color assignment based on the composite stress proxy of Equation~\eqref{eq:stress_proxy}.}
...
\caption{Viterbi regime sequence for INTC. Green $=$ Calm (R0, 80.6\%), blue $=$ Intermediate (R1, 13.3\%), red $=$ Stressed (R2, 6.1\%).}
...
\caption{Viterbi regime sequence for GOOG. Green $=$ Calm (R1, 1.3\%), blue $=$ Intermediate (R2, 3.6\%), red $=$ Baseline/active (R0, 95.1\%). The dominant R0 exhibits the highest composite microstructure activity within GOOG's own distribution; see text for interpretation.}
...
\caption{Viterbi regime sequence for AMZN. Green $=$ Calm (R2, 84.1\%), blue $=$ Intermediate (R0, 9.9\%), red $=$ Stressed (R1, 6.0\%). R1 constitutes a clear volatility shock ($\sigma_r = 1.09$\,bp).}
...
\caption{Viterbi regime sequence for MSFT. Green $=$ Calm (R2, 3.3\%), blue $=$ Intermediate (R1, 78.1\%), red $=$ Stressed (R0, 18.6\%). Regime separation is driven primarily by $|\text{OFI}|$ rather than return volatility.}
...
Table~\ref{tab:local_global_sync} presents the synchronization between each ticker's local HMM and the Meta-HMM global regime, across 83 global transitions. Two distinct patterns emerge. \textbf{Early-warning tickers} (AMZN and INTC) exhibit zero raw synchronization---their local transitions never coincide with the global transition window---yet carry the highest leadership scores (0.60 and 0.50 respectively), meaning their local state changes reliably \textit{precede} global shifts by a few observations. This combination of zero co-occurrence with high lead count implies that AMZN and INTC local transitions are predictive leading indicators that fire before the sector-wide shift materializes, but not simultaneously with it. \textbf{AAPL} combines a moderate synchronization rate (10.8\%, 9 co-transitions out of 83) with a high leadership score (0.481), making it the ticker most directly coupled to global dynamics: its local regime shifts both anticipate and occasionally coincide with sector-wide transitions. \textbf{GOOG} shows limited synchronization (6.0\%) with a moderate leadership score (0.333). \textbf{MSFT}, despite accumulating the most raw lead events (21 leads, 16 lags), converts them into the lowest leadership score (0.135), indicating that many of its local transitions are noise uncorrelated with the global pattern.
...
\paragraph{Unconditional cross-ticker lead-lag.}
...
\paragraph{Stress-conditioned topology: calm vs.\ high-stress.}
Table~\ref{tab:leadlag_between_tickers} presents the quantile-conditioned analysis, which reveals that the lead-lag network topology is state-dependent and changes materially between calm and stressed regimes. In the \textbf{low-stress regime (Q10)}, GOOG emerges as the dominant leading indicator: GOOG leads AAPL by $+2$\,s ($r = 0.393$), leads MSFT by anticipating it $8$\,s ahead ($r = 0.278$), and leads INTC ($r = 0.160$). Simultaneously, AMZN and MSFT are found to \emph{precede} GOOG in calm conditions. This ``hub'' role of GOOG during calm periods is consistent with informed price discovery in a low-noise environment, where the most liquid mega-cap stock sets the directional tone for the sector. In the \textbf{high-stress regime (Q90)}, the network inverts: \textbf{INTC becomes the dominant leader}. INTC anticipates AAPL by 7.5\,s ($r = 0.526$, the strongest pairwise correlation across all quantiles and pairs), anticipates MSFT near-instantaneously ($r = 0.387$), and also leads AAPL at $+10$\,s ($r = 0.381$). AMZN also leads AAPL during stress ($r = 0.372$, $+2.5$\,s). Q90 correlations substantially exceed Q10 across all pairs, confirming that microstructure interdependencies amplify nonlinearly under market stress---a pattern that static correlation measures conflate with heteroskedasticity, whereas the Wasserstein-regime framework reveals as a genuine structural shift in the causal network.
...
Table~\ref{tab:transfer_entropy} presents the top directed information flow relationships between tickers, computed from regime posterior probabilities. The dominant link is \textbf{AAPL$\rightarrow$MSFT} (TE $= 0.0060$ nats), which is more than twice the second-strongest flow---by a substantial margin. MSFT emerges as a central hub: it both receives from AAPL (0.0060) and transmits back to AAPL (0.0028), receives from AMZN (0.0025), and sends to AMZN (0.0022). The AAPL--MSFT and AMZN--MSFT channels thus form the two dominant bidirectional axes of the regime contagion network. This result is economically intuitive: AAPL and MSFT are the two largest-cap stocks in the dataset with deep, actively managed institutional overlap, making their regime dynamics highly coupled. The AMZN$\rightarrow$AAPL (0.0021) and AAPL$\rightarrow$AMZN (0.0020) pair constitutes a third symmetric channel, while INTC and GOOG are secondary transmitters---INTC primarily into AAPL (0.0014) and MSFT (0.0014), GOOG into AAPL (0.0015) and AMZN (0.0014).

\input{tables/transfer_entropy_top.tex}

\paragraph{Patient Zero identification.}
Combining the TE matrix with the synchronization leadership scores, \textbf{AAPL is the leading Patient Zero candidate}. It has the highest total outgoing TE across all four targets (AAPL$\rightarrow$\{INTC, GOOG, AMZN, MSFT\} $= 0.0093$ nats), the highest co-transition synchronization rate (10.8\%), and a high leadership score (0.481) indicating that its local transitions systematically precede global shifts. AMZN presents a complementary profile: highest pure leadership score (0.600) but moderate outgoing TE (0.0050 nats) and zero co-transition synchronization---consistent with AMZN firing leading signals before global transitions without directly causing them. The picture that emerges is therefore a two-tier initiator structure: AMZN acts as an \emph{anticipatory signal} several observations before the global regime shift, while AAPL acts as the \emph{direct causal trigger} whose regime transition both coincides with and propagates the sector-wide change through the dominant AAPL$\rightarrow$MSFT channel. INTC contributes as a secondary amplifier, particularly in the high-stress Q90 lead-lag network where it leads AAPL by $7.5$\,s with $r = 0.526$. This layered causal structure---AMZN as early-warning, AAPL as trigger, MSFT as primary recipient, INTC as stress amplifier---provides a richer and more actionable picture of intra-sector contagion than symmetric correlation analysis.
...
The weak but structured global synchronization is consistent with a non-crisis day. The low synchronization rates (0--10.8\%) and moderate ARI (0.150--0.476) indicate that idiosyncratic microstructure dynamics dominate sector-wide co-movement. The discovery that AMZN and INTC are pure early-warning tickers (high leadership, zero co-transition) while AAPL directly triggers global transitions provides a nuanced baseline: on a crisis day, one would expect this two-tier structure to collapse into simultaneous co-transitions as common shocks override idiosyncratic dynamics.

Transfer Entropy reveals that the dominant contagion channel is AAPL$\rightarrow$MSFT (0.0060 nats), not the AAPL--INTC pair as prior analysis suggested. MSFT's role as the central hub---receiving from both AAPL and AMZN, and retransmitting---is consistent with MSFT's position as the most broadly held institutional stock in the dataset. The directed TE structure, combined with the stress-conditioned lead-lag evidence that INTC leads AAPL by $7.5$\,s at Q90, reveals a causal topology that is both asymmetric and state-dependent---neither of which is detectable from unconditional correlation matrices.
...
Tables~\ref{tab:regime_stats_aapl}--\ref{tab:regime_stats_msft} report descriptive
statistics of the original microstructure features (price return volatility $\sigma_r$,
mean absolute return $|\bar{r}|$, return excess kurtosis, mean absolute OBI, and mean
absolute OFI) broken down by HMM regime and ticker. Regimes are ordered and labelled
by the composite stress proxy of Equation~\eqref{eq:stress_proxy}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/leadlag_interticker_heatmap.png}
